  ...
  return (
    <div ref={ref} className={`relative ${className}`}>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex min-w-[10rem] items-center justify-between gap-2 rounded-xl border border-[#111111]/15 bg-white px-4 py-2.5 text-left text-sm font-semibold text-[#111111] outline-none transition-colors hover:border-[#122428]/40 focus:border-[#122428]"
      >
          ...
          {options.map((opt) => (
            <button
              ...
              className={`flex w-full items-center gap-2 px-3 py-2.5 text-left text-sm transition-colors hover:bg-[#F5F8F8] ${
                value === opt.value ? "bg-[#122428]/8 font-semibold text-[#122428]" : "text-[#111111]"
              }`}
            >
