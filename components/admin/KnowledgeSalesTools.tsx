      ...
      <div className="rounded-xl border border-[#111111]/10 p-5">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="flex shrink-0 items-center gap-3">
            ...
            <button
              ...
              className="rounded-lg border border-[#111111]/15 px-3 py-1.5 text-xs font-semibold text-[#6f6b62] transition-colors hover:border-[#063b32] hover:text-[#063b32]"
            >
              Edit
            </button>
          </div>
        ...
        {rule.inclusions && rule.inclusions.length > 0 && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {rule.inclusions.map((inclusion) => (
              <span key={inclusion} className="rounded-full bg-[#f7f4ea] px-2.5 py-0.5 text-[10px] font-semibold text-[#6f6b62]">{inclusion}</span>
            ))}
          </div>
        )}
      </div>
    ...
    <div className="space-y-3 rounded-xl border border-[#063b32]/30 p-5">
      <div className="grid grid-cols-2 gap-3">
        <label className="text-[10px] font-semibold uppercase tracking-wider text-[#6f6b62]">
          Name
          <input value={draft.name} onChange={(e) => setDraft((d) => ({ ...d, name: e.target.value }))} className="mt-1 w-full rounded-lg border border-[#111111]/15 px-3 py-1.5 text-sm normal-case tracking-normal text-[#111111] outline-none focus:border-[#063b32]" />
        </label>
        <label className="text-[10px] font-semibold uppercase tracking-wider text-[#6f6b62]">
          Category
          <input value={draft.category} onChange={(e) => setDraft((d) => ({ ...d, category: e.target.value }))} className="mt-1 w-full rounded-lg border border-[#111111]/15 px-3 py-1.5 text-sm normal-case tracking-normal text-[#111111] outline-none focus:border-[#063b32]" />
        </label>
      </div>
      <label className="block text-[10px] font-semibold uppercase tracking-wider text-[#6f6b62]">
        Unit
        <input value={draft.unit} onChange={(e) => setDraft((d) => ({ ...d, unit: e.target.value }))} className="mt-1 w-full rounded-lg border border-[#111111]/15 px-3 py-1.5 text-sm normal-case tracking-normal text-[#111111] outline-none focus:border-[#063b32]" />
      </label>
      <div className="grid grid-cols-3 gap-3">
        {([
          ...
          ["band_high", "Band high (£)"],
        ] as [keyof Pick<PricingDraft, "band_low" | "band_expected" | "band_high">, string][]).map(([key, label]) => (
          <label key={key} className="text-[10px] font-semibold uppercase tracking-wider text-[#6f6b62]">
            {label}
            <input type="number" value={draft[key]} onChange={(e) => setDraft((d) => ({ ...d, [key]: e.target.value }))} className="mt-1 w-full rounded-lg border border-[#111111]/15 px-3 py-1.5 text-sm normal-case tracking-normal text-[#111111] outline-none focus:border-[#063b32]" />
          </label>
        ))}
      </div>
      <label className="block text-[10px] font-semibold uppercase tracking-wider text-[#6f6b62]">
        Description
        <textarea value={draft.description} onChange={(e) => setDraft((d) => ({ ...d, description: e.target.value }))} rows={2} className="mt-1 w-full resize-none rounded-lg border border-[#111111]/15 px-3 py-1.5 text-sm normal-case tracking-normal text-[#111111] outline-none focus:border-[#063b32]" />
      </label>
      <div className="flex justify-end gap-2 pt-1">
        <button type="button" onClick={() => { resetDraft(); setEditing(false); }} disabled={saving} className="flex items-center gap-1.5 rounded-lg border border-[#111111]/15 px-3 py-1.5 text-xs font-semibold text-[#6f6b62] hover:bg-[#f7f4ea]">
          <X className="h-3.5 w-3.5" /> Cancel
        </button>
        <button type="button" onClick={() => void save()} disabled={saving} className="flex items-center gap-1.5 rounded-lg bg-[#063b32] px-3 py-1.5 text-xs font-semibold text-white disabled:opacity-50">
    ...
    <div>
      ...
      {loading ? <Loading /> : scripts.length === 0 ? <Empty>No scripts found.</Empty> : (
        <div className="space-y-3">
          {scripts.map((script) => (
            <div key={script.id} className="rounded-xl border border-[#111111]/10 bg-white p-5">
              <div className="mb-3 flex items-start justify-between gap-3">
                ...
                <CopyButton copied={copied === script.id} onClick={() => copyToClipboard(script.content, setCopied, script.id)} />
              </div>
...
export function ObjectionsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    setObjections(dedupeBy(json.data || [], (objection) => [
      normalize(objection.objection), normalize(objection.response), normalize(objection.category),
    ].join("|")));
    setLoading(false);
  }, [search, category]);
      ...
      {loading ? <Loading /> : objections.length === 0 ? <Empty>No objections found.</Empty> : (
        <div className="space-y-3">
          {objections.map((objection) => (
            <div key={objection.id} className="rounded-xl border border-[#111111]/10 bg-white p-5">
              <div className="flex items-start justify-between gap-3">
                ...
                <CopyButton copied={copied === objection.id} onClick={() => copyToClipboard(objection.response, setCopied, objection.id)} />
              </div>
            </div>
          ))}
        </div>
...
function Loading() {
  return <div className="py-16 text-center text-sm text-[#6f6b62]">Loading…</div>;
}

function Empty({ children }: { children: React.ReactNode }) {
  return <div className="rounded-xl border border-[#111111]/10 py-12 text-center text-sm text-[#6f6b62]">{children}</div>;
}
