  ...
  if (msg.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[85%] rounded-2xl rounded-br-md bg-[#063b32] px-3.5 py-2.5 text-[13px] leading-relaxed text-white whitespace-pre-wrap break-words">
          {msg.content}
        </div>
      </div>
    );
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/40 p-4">
      <div className="flex max-h-[min(720px,90vh)] w-full max-w-2xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex shrink-0 items-center gap-3 border-b border-[#111111]/10 bg-[#0a1f18] px-5 py-4">
          <div className="grid h-8 w-8 place-items-center rounded-full bg-[#f5f274]">
            <Sparkles className="h-4 w-4 text-[#0a1f18]" />
          </div>
        ...
        <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto px-5 py-4 space-y-4">
          {loading ? (
            ...
            <p className="py-8 text-center text-sm text-[#6f6b62]">No messages in this chat.</p>
          ) : (
