      ...
      <div className="space-y-4">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="rounded-md bg-[#063b32] px-4 py-2 text-sm font-semibold text-white disabled:opacity-50"
          >
            {saving ? "Saving…" : "Save author"}
          </button>
          <button type="button" onClick={onCancel} className="rounded-md border border-[#111111]/15 px-4 py-2 text-sm font-semibold text-[#6f6b62]">
            Cancel
          </button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white">
      <div className="border-b border-[#111111]/10 bg-white px-8 py-6">
        ...
        <p className="mt-0.5 text-sm text-[#6f6b62]">Create author profiles to attribute posts. Each profile is shown on the published post.</p>
      </div>

      <div className="mx-auto max-w-2xl px-8 py-8">
        <div className="space-y-4">
          {loading ? (
            <div className="py-12 text-center text-sm text-[#6f6b62]">Loading…</div>
          ) : (
            authors.map((author) =>
              editingId === author.id ? (
                <AuthorForm
                ...
                >
                  {author.avatar_url ? (
                    ...
                    <div className="grid h-12 w-12 shrink-0 place-items-center rounded-full bg-[#063b32] text-lg font-semibold text-[#f5f274]">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {author.bio && (
                      <p className="mt-1 text-sm leading-5 text-[#6f6b62] line-clamp-2">{author.bio}</p>
                    )}
                  </div>
                  <div className="flex shrink-0 gap-1">
                    <button
                      onClick={() => setEditingId(author.id)}
                      className="grid h-8 w-8 place-items-center rounded-md text-[#6f6b62] hover:bg-[#f7f4ea] hover:text-[#111111]"
                    >
                      <Edit2 className="h-3.5 w-3.5" />
                    </button>
                    <button
                      onClick={() => deleteAuthor(author.id)}
                      className="grid h-8 w-8 place-items-center rounded-md text-[#6f6b62] hover:bg-red-50 hover:text-red-600"
                    >
          ...
          {!showForm && !editingId && (
            <button
              ...
              className="flex w-full items-center justify-center gap-2 rounded-md border border-dashed border-[#111111]/20 bg-white py-4 text-sm font-semibold text-[#6f6b62] hover:border-[#063b32]/40 hover:text-[#063b32]"
            >
