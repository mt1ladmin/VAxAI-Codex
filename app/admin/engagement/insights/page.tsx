...
export default function InsightsPage() {
  ...
  const load = useCallback(async () => {
    setLoading(true);
    const [oppsRes, tasksRes, orgsRes, contactsRes, intRes] = await Promise.all([
      fetch("/api/admin/engagement/opportunities?limit=500"),
      fetch("/api/admin/engagement/tasks?status=todo&limit=200"),
      fetch("/api/admin/engagement/organisations?limit=1&status=all"),
      fetch("/api/admin/engagement/contacts?limit=1"),
      fetch("/api/admin/engagement/interactions?limit=1"),
    ]);
      ...
      {loading ? (
        ...
        <div className="px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            {[
              { label: "Opportunities", value: stats?.totalOpps ?? 0, href: "/admin/engagement/pipeline/opportunities" },
              { label: "Won / clients", value: stats?.wonCount ?? 0, href: "/admin/engagement/pipeline" },
              { label: "Pipeline value", value: stats?.pipelineValue ? `£${(stats.pipelineValue).toLocaleString()}` : "—", href: "/admin/engagement/pipeline" },
          ...
          <div className="rounded-xl border border-[#111111]/10 p-6">
            <div className="flex items-center justify-between mb-5">
              ...
              <Link href="/admin/engagement/pipeline" className="flex items-center gap-1 text-xs font-semibold text-[#063b32] hover:underline">
                <TrendingUp className="h-3.5 w-3.5" /> View pipeline
              </Link>
            </div>
            <div className="space-y-3">
              {byStage.map(({ stage, count, value }) => (
                <Link key={stage} href={`/admin/engagement/pipeline/opportunities?stage=${encodeURIComponent(stage)}`} className="block group">
